...
public class CallResumeEventsJoiner implements AbstractCallEventsJoiner {
    ...
    /**
     * Joins resume Events in callEvent cache and callBackMetaEvent cache (if present)
    ...
    public void join(String vendorCallId) {
        ...
        Optional<CallBackMetaEventDto> optionalCallBackMetaEventDto = callBackMetaEventsRedisStore.optionalGet(vendorCallId, CallBackMetaEventType.RESUME_EVENT.name());
        ...
        log.info("VendorCallId: {}, Performing join between callEventDto and callBackMetaEvent for resume event", callEventDto.getVendorCallId());
        ...
        if (callStartMessageOptional.isEmpty()) {
            log.info("ObserveCallId: {}, VendorCallId: {}, Call start message not found for resume event",
                    callEventDto.getObserveCallId(), vendorCallId);
            return;
        }
        CallStartMessage startMessage = callStartMessageOptional.get();
        publishResumeMessage(callEventDto, startMessage);
    }

    private void publishResumeMessage(CallEventDto callEventDto, CallStartMessage startMessage){
        if (startMessage.isComplete()) {
            log.info("ObserveCallId: {}, Call resume message is complete after join. "
                    + "publishing resume message to kafka.", callEventDto.getObserveCallId());
            publishCallEvent(callEventDto.getObserveCallId(), callEventDto.getObserveAccountId(), callEventDto.getObserveUserId(), "RESUME");
            updateCallStatus(callEventDto.getObserveCallId());
        } else {
            log.warn("ObserveCallId: {}, Call start message is still partial even after join, ignoring resume message. StartMessage: {}",
    ...
    private void updateCallStatus(String observeCallId){
        callStatusRedisStore.push(observeCallId, CallStatus.RESUME);
    }
